...
func GetAvatar(w http.ResponseWriter, r *http.Request) {
	...
	opFile, err := os.Open("uploads/avatars/" + profile.Avatar)
	if err != nil {
		http.Error(w, "cannot find the user avatar ", http.StatusBadRequest)
	}
	//if the avatar img is founded, Copy send to the writer the binary file
	_, err = io.Copy(w, opFile)
	if err != nil {
		http.Error(w, "Error in avatar copy", http.StatusBadRequest)
